...
def consensus_fasta(path, fasta, n_ind):  # modified from biopython  #add bases degeneradas
    alig = AlignIO.read(path + fasta, 'fasta')
    #    print alig
    consensus = []
...
def diagnoser(path, n_spec, n_ind):
    lists_var = []
    for i in range(n_spec):
        lists_var.append(consensus_fasta(path + 'tmp/', 'spec' + str(i) + '.fasta', n_ind))
    list_all = []
            ...
            if lists_var[i] != None:
                ...
                for d in range(len(list_tmp)):
                    if list_tmp[d] == 1 and n_var[d] == 1:
                        part += 1
                if count == 1 and N == 0 and len(lists_var[i][j]) == 1 and lists_var[i][j] != ['N']:
                    list_D.append(1)  # 1 Diagnostic
                elif count == 1 and N == 1 and len(lists_var[i][j]) == 1 and lists_var[i][j] != ['N']:
                    list_D.append(2)  # 2 Diagnostic or partial
                elif count > 1 and N == 0 and part == 1 and len(lists_var[i][j]) == 1 and lists_var[i][j] != ['N']:
                    list_D.append(3)  # 3 Partial
                elif count > 1 and N == 1 and part == 1 and len(lists_var[i][j]) == 1 and lists_var[i][j] != ['N']:
                    list_D.append(4)  # 4 Partial or uniformative
                elif count == 1 and N == 1 and len(lists_var[i][j]) == 1 and lists_var[i][j] != ['N'] and n_only >= 1:
                    list_D.append(5)  # 5 Invalid
                elif count > 1 and N == 1 and part == 1 and len(lists_var[i][j]) == 1 and lists_var[i][j] != [
                    'N'] and n_only >= 1:
                    list_D.append(5)  # 5 Invalid
                else:
...
def summary(path, list_all, spec_list, n_ind_spec, n_ind):
    with open(path + 'Diagnostic_character.txt', 'w+') as diagnostic:
        ...
        for k in range(len(spec_list2)):
            ...
            for j in list_all:
                if j[k] == 1:
                    ...
                    z_sum += 1
            u.append(u_sum), w.append(w_sum), x.append(x_sum), y.append(y_sum), z.append(z_sum)
        ...
        df['Invalid'] = z
        df1 = df[['Group name', 'N', 'Diagnostic', 'Diag. or Partial', 'Partial', 'Part. or Uninformative',
                  'Invalid']].copy()
...
def plot_DC(path, list_var, cod_color, spec_list, list_n_pos):
    ...
    #    print list_var
    list_var2 = []
    list_n_pos = [list_n_pos[x] for x in range(len(cod_color)) if sum(cod_color[x]) != 0]
    pos_cc = 0
    pos_cc_ls = []
    #    print cod_color
    ...
    list_var = list_var2
    #    print list_var
    cod_color = [cod_color[x] for x in range(len(cod_color)) if sum(cod_color[x]) != 0]
    ...
    for i in range(int(ceil(len(pos_cc_ls) / float(n_div)))):  # 0 1 2
        ...
        cod_color2 = [[
            'red' if x == 1 else '#ff5b00' if x == 2 else 'orange' if x == 3 else 'yellow' if x == 4 else '#feffca' if x == 4 else 'white'
            for x in l] for l in cod_color2]
        dict_color = {}
        ...
        trace = go.Table(
            ...
            domain=dict(x=[0, float(width_1 + (width_2 * len(list_n_pos_tmp))) / (width_1 + (width_2 * n_div))],
                        y=[yvalue, sz - 0.02]),  ####
        ...
        lists_data[i] = trace
        print([0, float(width_1 + (width_2 * len(list_n_pos_tmp))) / (width_1 + (width_2 * n_div))],
              [yvalue, sz - 0.02])
        sz = sz - ((len(spec_list) + 1) * fct)

    data_lgnd = {'color': ['', '', '', '', ''],
                 'Legend': ['Diagnostic Character', 'Diagnostic or Partial Character', 'Partial Character',
                            'Partial or Uninformative Character', 'Invalid Character']}
    ...
    trace_legend = go.Table(
        ...
        header=dict(values=['', '<b>Legend</b>'],
                    ...
                    align=['left', 'center'],
                    font=dict(color='black', size=11)),
        cells=dict(values=[df2.color, df2.Legend],
                   line=dict(color='#506784'),
    ...
    plotly.offline.plot(fig, filename=path + 'Diagnostic_character.html', auto_open=False,
