...
def diagnoser(path,fasta,n_spec,n_ind):
    lists_var=[]
    for i in range(n_spec):
...
#                    print lists_var[i][j]
                    ...
                    elif count>1 and N==1 and part==1 and len(lists_var[i][j])==1 and lists_var[i][j]!=['N'] and n_only>=1:
                        list_D.append(5) #5 Invalid
                    else:
...
def summary(path,list_all,spec_list,n_ind_spec,n_ind):
    with open(path+'Diagnostic_character.txt','w+') as diagnostic:
        ...
        df['Invalid']=z  
        df1=df[['Group name','N','Diagnostic','Diag. or Partial','Partial','Part. or Uninformative','Invalid']].copy()
...
#    Code for do the tables with the data prepared above       
    ...
    for i in range(int(ceil(len(pos_cc_ls)/float(n_div)))): #0 1 2
        ...
        cod_color2=[['red' if x == 1 else '#ff5b00' if x==2 else 'orange' if x==3 else 'yellow' if x==4 else '#feffca' if x==4 else 'white' for x in l] for l in cod_color2]       
        dict_color={}
    ...
    plotly.offline.plot(fig, filename = path+'Diagnostic_character.html',auto_open=False, config={'displayModeBar': False},show_link=False)
...
def main(path,fasta,n_ind=3,gen=1,sp=2):
    ...
    n_ind_spec=n_spec[2]
    n_spec=n_spec[0]
    list_all=diagnoser(path,fasta,n_spec,n_ind)
