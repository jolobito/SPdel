...
class Matrian:
    ...
    def __init__(self, path, fasta, gen, sp, distance):
        ...
        self.fasta_names = [seq.id for seq in self.fasta_seqIO]  # list of samples names
        self.fasta_dict = SeqIO.to_dict(SeqIO.parse(path + fasta, "fasta"))  # dictionary of sequences
    ...
    def matrix(self, distance):  # genera la matriz de distancia
        ...
        for i in self.fasta_names:
            for j in self.fasta_names:
                if distance == 'k':
                    num = self.k2Pdistance(self.fasta_dict[i].seq, self.fasta_dict[j].seq)
                    if num == -0.0:
                        num = 0.0
                    dmatrix[i][j] = (round(num, 5))  # code by neguinha!
                if distance == 'p':
                    num = self.pdistance(self.fasta_dict[i].seq, self.fasta_dict[j].seq)
                    if num == -0.0:
                        num = 0.0
                    dmatrix[i][j] = (round(num, 5))  # code by neguinha!
        return dmatrix
    ...
    def mean_intra(self):  # da media intra de todos los grupos 
        ...
        for sp in self.Lname:
            ...
            for ind in ind_sp:
                ...
                L = L + [values for key, values in ind_dic.items() if key.startswith(sp) and key is not ind]
            if L == []:
                v = None
            else:
                v = sum(L) / len(L)
    ...
    def max_intra(self):  # da maxima dentro de un grupo
        ...
        for sp in self.Lname:
            ...
            for ind in ind_sp:
                ind_dic = self.data[ind]
                L = L + [values for key, values in ind_dic.items() if key.startswith(sp)]
            v = max(L)
            max_intra_dict[sp] = v
    ...
    def min_inter(self):  # da minima inter de um grupo 
        ...
        NN_dict = {}
        for sp in self.Lname:
            ind_sp = [ind for ind in self.fasta_names if ind.startswith(sp)]  # list of individuals of a given sp
            L = []
            K = []
            for ind in ind_sp:
                ind_dic = self.data[ind]  # dictionary of individuals genetic distances
                L = L + [values for key, values in ind_dic.items() if not key.startswith(sp)]
                K = K + [key for key in ind_dic.keys() if not key.startswith(sp)]
            v = min(L)
            kpos = L.index(v)
            min_inter_dict[sp] = v
            NN_dict[sp] = K[kpos]
    ...
    def save_csv(self, out_name):
        ...
        with open(out_name_csv, 'w', newline='') as out_file:
            ...
            for r in self.fasta_names:
                writer.writerow([r] + [value for value in self.data[r].values()])
