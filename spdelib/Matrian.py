...
class matrian:  # lower matrix
    ...
    def save_csv(self, out_name):
        ...
        with open(out_name_csv, 'w') as out_file:
            writer = csv.writer(out_file, delimiter=',')
            writer.writerow(['name'] + self._fasta_sp)
            i = 0
            for r in self._fasta_sp:
                writer.writerow([r] + self.data[i])
                i += 1
