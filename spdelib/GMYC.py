        ...
        for node in self.nodes:
            ...
            if times >= 0:
                ...
                if reach_t:
                    ...
                    wt = waiting_time(
                        length=times, num_coas=curr_num_coa, num_lines=curr_spe
                    )
                    ...
                    if node.id == threshold_node.id:
                        ...
                        tcnt = 0
                        wt = waiting_time(
                            length=times, num_coas=0, num_lines=curr_spe
                        )
                        num_spe = curr_spe
                        curr_spe = curr_spe - 1
                        curr_num_coa = 2
        ...
        if self.num_lineages <= 0:
            ...
            try:
                bp = (
                    self.p
                    * self.rate
                    * math.pow(self.num_lineages, (self.p - 1.0))
                )
            except ValueError:
                bp = 0
...
class coalescent:
    ...
    def getCoalesecntRate(self):
        rv = 0
        try:
            rv = self.rate * math.pow(
                self.num_individual * (self.num_individual - 1.0), self.p
            )
        except ValueError:
            rv = 0
        return rv
    ...
    def bprime(self):
        ...
        try:
            bp = (
                ...
                * self.rate
                * math.pow(
                    self.num_individual * (self.num_individual - 1.0),
                    (self.p - 1.0),
                )
            )
        except ValueError:
...
class coalescents:
    def __init__(
        self, num_coalescent, rate=0, p=1, const_rate=True, const_p=True
    ):
        self.coa_list = []
...
class waiting_time:
        ...
        for coa in self.coas.coa_list:
            try:
                br = (
                    br
                    + math.pow(
                        coa.num_individual * (coa.num_individual - 1),
                        self.coa_p,
                    )
                    * self.length
                )
    ...
    def bprime_spe(self):
        bp = self.spe.bprime()
        bps = (
            bp * math.exp(-1.0 * self.b * self.length)
            + self.b
            * math.exp(-1.0 * self.b * self.length)
            * -1.0
            * self.length
            * bp
        )
...
class tree_time:
    ...
    def show(self):
        print(
            (
                "This is tree_time with spe event: "
                + repr(self.numSpeEvent)
                + ", coa event: "
                + repr(self.numCoaEvent)
            )
        )
...
def call_upgma(fin):
    ...
    call(
        [basepath + "/bin/FastTree.exe", "-nt", fin],
        stdout=open(fin + ".upgmaout", "w"),
...
def main(argu=None):
    ...
    try:
        ...
        sp = gmyc(
            ...
            show_llh=sshow_llh,
            show_lineages=sshow_lineages,
            print_species=sprint_species,
            pv=p_value,
        )
