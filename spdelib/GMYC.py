...
try:
    ...
    import os
    import subprocess
    from ete3 import Tree, NodeStyle, TreeStyle, TextFace, SeqGroup
...
class coalescents:
    ...
    def __str__(self):
        ...
        for coa in self.coa_list:
            s = s + "coa_event" + repr(cnt) + ", with n= " + repr(coa.num_individual) + ", rate=" + repr(
                coa.rate) + "\n"
            cnt = cnt + 1
        return s
...
class waiting_time:
    ...
    def bprime_spe(self):
        bp = self.spe.bprime()
        bps = bp * math.exp(-1.0 * self.b * self.length) + self.b * math.exp(
            -1.0 * self.b * self.length) * -1.0 * self.length * bp
        return bps
...
class tree_time:
    ...
    def show(self):
        print(
            ("This is tree_time with spe event: " + repr(self.numSpeEvent) + ", coa event: " + repr(self.numCoaEvent)))
...
class null_model:
    ...
    def logl(self, p=1.0):
        ...
        for wt in self.wt_list:
            try:
                logl = logl + math.log(self.rate * math.pow(wt.num_lines, self.p) * math.exp(
                    self.rate * math.pow(wt.num_lines, self.p) * wt.length * -1.0))
            except ValueError:
                pass
...
def optimize_null_model(umtree):
    ...
    while change > min_change and cnt < max_iters:
        ...
        para, nn, cc = fmin_tnc(tar_fun_null, [0, 0], args=[nm], disp=False, bounds=[[0, 10], [0, 10]],
                                approx_grad=True)
...
def gmyc(tree, print_detail=False, show_tree=False, show_llh=False, show_lineages=False, print_species=False, pv=0.01):
    ...
    #		utree.tree.show()  #######################
    #	else: #######################
    #		utree.tree.render('/GMYC'+tree+".png") #######################
    #		utree.tree.render('/GMYC'+tree+".pdf")  #######################
...
def gmyc_func(tree, taxa_order, print_detail=False, show_tree=False, show_llh=False, show_lineages=False,
              print_species=False, pv=0.01):
    llh_list = []
    min_change = 0.1
    max_iters = 100
    best_llh = float("-inf")
...
def call_upgma(fin):
    ...
    call([basepath + "/bin/FastTree.exe", "-nt", fin], stdout=open(fin + ".upgmaout", "w"), stderr=subprocess.STDOUT)
...
def print_options():
    ...
    print(
        "    -pvalue (0-1)                    Set the p-value for likelihood ratio test with TWO degrees of freedom.(default 0.01)")
    print(
        "                                     Note the old split package used three degrees of freedom which has been proven wrong.\n")
...
def main(argu=None):
    ...
    if len(sys.argv) < 3 and (argu is None):
        print_options()
        sys.exit()
    if argu is not None:
        for i in argu:
    ...
    if salignment != "":
        basepath = os.path.dirname(os.path.abspath(__file__))
        if not os.path.exists(basepath + "/bin/FastTree.exe"):
            ...
            print(
                "Please complie with gcc -O3 -finline-functions -funroll-loops -Wall -o FastTree FastTreeUPGMA.c -lm, ")
    ...
    try:
        ...
        sp = gmyc(tree=stree, print_detail=sprint_detail, show_tree=sshow_tree, show_llh=sshow_llh,
                  show_lineages=sshow_lineages, print_species=sprint_species, pv=p_value)
